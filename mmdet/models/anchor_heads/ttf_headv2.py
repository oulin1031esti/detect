...
###
# File: ttf_headv2.py
# Created Date: Saturday, August 8th 2020, 4:44:07 pm
# Author: yusnows
# -----
# Last Modified:
# Modified By:
...
class FPN(nn.Module):
    ...
    def forward(self, input):
        ...
        up4 = F.interpolate(output4, size=[output3.shape[2], output3.shape[3]], mode="nearest")
        output3 = output3 + up4
...
class TTFHeadv2(AnchorHead):
    def __init__(
        ...
        self.head_hm = nn.Conv2d(ssh_outplane, num_classes+1, 1)
        self.head_wh = nn.Conv2d(ssh_outplane, 4, 1)
        self.head_hm.bias.data.fill_(-2.19)

    def init_weights(self):
        for _, m in self.fpn.named_modules():
            if isinstance(m, nn.Conv2d):
                kaiming_init(m)

        for _, m in self.asff.named_modules():
            if isinstance(m, nn.BatchNorm2d):
                nn.init.constant_(m.weight, 1)
                nn.init.constant_(m.bias, 0)

        for _, m in self.ssh.named_modules():
            if isinstance(m, nn.Conv2d):
                normal_init(m, std=0.01)

        bias_cls = bias_init_with_prob(0.01)
        normal_init(self.head_hm[-1], std=0.01, bias=bias_cls)

        for _, m in self.head_wh.named_modules():
            if isinstance(m, nn.Conv2d):
                normal_init(m, std=0.001)

    def forward(self, feats):
        out1, out2, out3, out4 = self.fpn(feats)
